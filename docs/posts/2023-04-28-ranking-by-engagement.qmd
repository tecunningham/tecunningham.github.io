...
- Ravi note: https://psychoftech.substack.com/p/when-should-companies-optimize-for (Matt Motyl says was based on note they both wrote at FB)
...
#| fig.align: center
#| column: margin
\begin{tikzpicture}[scale=4, line width=1]
   ...
   %\draw[->] (.3.8)--(.5,1) node[above]{retentiveness};
...
\begin{tikzpicture}[scale=6, line width=1]
   ...
   \draw[->] (.2,.9) -- node[above,rotate=45]{retentiveness} (.4,1.1);
   \draw[dotted,line width=.5] (.8,.3) -- (.2,.9);% node[right,rotate=90]{retentiveness low};
   \draw[dotted,line width=.5] (.8,.5) -- (.4,.9);% node[right,rotate=90]{retentiveness med};
   \draw[dotted,line width=.5] (.8,.7) -- (.6,.9);% node[right,rotate=90]{retentiveness high};
   \fill[blue] (.5,.5) circle (.02) node[below]{unranked};
   \fill[red!50!black] (.8,.39) circle (.02) node[right]{\ max engagement};
   \fill[green!30!black] (.68,.68) circle (.02) node[right]{\ max engagement and quality};
   \fill[green!30!black] (.39,.8) circle (.02) node[above]{\ max quality};
\end{tikzpicture}
```
...
Here I describe a few formal properties of a model of ranking based on a joint-normal distribution of attributes. Fuller proofs of some of these results are [here](https://tecunningham.github.io/posts/2023-10-23-pareto-frontiers-experiments-ranking.html), other are in notes I'm preparing and available on request.
...
**@milli2021optimizing (2021) "From Optimizing Engagement to Measuring Value"**
