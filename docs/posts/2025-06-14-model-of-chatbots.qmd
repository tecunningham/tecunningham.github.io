...
(simplest model: just lookup tables).
...
\begin{tikzpicture}[scale=4]
  ...
  % Central circle radius (where the centers lie)
  \def\RR{0.2}

  % Store radii for each circle for reference
  \def\rone{0.3}
  \def\rtwo{0.25}
  \def\rthree{0.2}
  \def\rfour{0.2}
  ...
  % Define and place each center (for convex hull)
  \coordinate (C1) at ({\RR*cos(0)}, {\RR*sin(0)});
  \coordinate (C2) at ({\RR*cos(60)}, {\RR*sin(60)});
  \coordinate (C3) at ({\RR*cos(120)}, {\RR*sin(120)});
  \coordinate (C4) at ({\RR*cos(180)}, {\RR*sin(180)});
  \coordinate (C5) at ({\RR*cos(240)}, {\RR*sin(240)});
  \coordinate (C6) at ({\RR*cos(300)}, {\RR*sin(300)});

  % Fill and draw interior dotted borders
  \fill[blue!20, opacity=0.35] (C1) circle (\rone);
  \fill[blue!20, opacity=0.35] (C2) circle (\rtwo);
  \fill[blue!20, opacity=0.35] (C3) circle (\rthree);
  \fill[blue!20, opacity=0.35] (C4) circle (\rfour);
  \fill[blue!20, opacity=0.35] (C5) circle (\rfive);
  \fill[blue!20, opacity=0.35] (C6) circle (\rsix);

  \draw[dotted,thick,blue!70] (C1) circle (\rone);
  \draw[dotted,thick,blue!70] (C2) circle (\rtwo);
  \draw[dotted,thick,blue!70] (C3) circle (\rthree);
  \draw[dotted,thick,blue!70] (C4) circle (\rfour);
  \draw[dotted,thick,blue!70] (C5) circle (\rfive);
  \draw[dotted,thick,blue!70] (C6) circle (\rsix);

  % Draw the guiding central circle (dashed, light gray)
  \draw[dashed,gray!60] (0,0) circle (\RR);

  % Draw convex hull using the outermost points of each circle in order
  % (calculate: point on each circle farthest from the center)
 ...
 \node at (0,.8) {Jagged frontier (union of learned knowledge)};
  % Optional: label the circles for clarity
  %\foreach \i in {1,...,6} {
  %  \node[font=\small] at (C\i) {$\i$};
  %}
\end{tikzpicture}
```
...
1. **Human-supervised:** LLMs can do tasks that are *close* to the set of tasks in the training set, but they're relatively worse outside.

2. **World-supervised:** works if (A) feedback is cheap; (B) has latent structure.

##    Implications

1. **Two types of human-supervised progress:**
   - more data
   - better extrapolation

2. **Will use LLMs for new-to-you tasks.** Will use for new-to-you questions.

3. **Will use LLMs for separable tasks.** Where the .

##    Implications for catastrophic risk

1. Things we're worried about:
   - AI R&D
   - Bioweapons
   - Hacking
   - Autonomy/self-replication

2. Two types of AI R&D progress
   - imitation learning
   - learning directly (AlphaZero).

3. Implication: it's useful to prune the training data to hinder capabilities.
