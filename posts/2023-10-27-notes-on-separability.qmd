...
**When is a set of choices consistent with an additively separable utility function?** Suppose you have a preference relation $\succeq$, over a multiattribute space, $X$, does there exist a set of functions $u_i:X\rightarrow\mathbb{R}$, such that for all $x,x'\in X$,
   $$x\succeq x' \iff \sum_{i=1}^n u_i(x_i) \geq \sum_{i=1}^n u_i(x'_i).$$
...
**What we want: a necessary and sufficient condition for a finite space.**
   ...
   - @wakker1989additive has a brief discussion on p33, states a cancellation condition.
...
\begin{tikzpicture}[color=red,line width=1]
   ...
   \draw (3,1) -- (1,3) node[midway,fill=white,sloped] {$\succ$}; 
   \draw[dotted] (3,2) -- (2,3) node[midway,fill=white,sloped] {$\succ$}; 
\end{tikzpicture}
```
   ...
   @wakker1988algebraic gives the Thomsen condition (for $n=2$) as:
...
```
   ...
   @karni1998hexagon define independence if *both*:
...
@wakker1989additive, this from @karni1998hexagon, for $n=2$:
...
**@bradley2005separable "Separable Discrete Preferences"**
   ...
   - They note that separability of $\succ$ is not sufficient for additivity of u when the outcome space is discrete. Their counter-example is from Kraft (1959).
   - They note that when outcomes are binary and additive then utility will be *linear*.
   - *Nice result! Separability _is_ sufficient for binary dimensions when n<=4.* (however $\delta$ isn't binary, if we're going to consider that as a dimension)
...
**Bergstrom **[Lecture Notes on Separable Preferences](https://econ.ucsb.edu/~tedb/Courses/GraduateTheoryUCSB/separabilitynotes.pdf)**
   ...
   - Theorem 3: with two commodities utility is additive separability if and only if double cancellation holds: u(x1,y1)>=u()
