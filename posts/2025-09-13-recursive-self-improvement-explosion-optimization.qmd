...
- Agents are starting to autonomously optimize real-world functions, beating the human state-of-the-art.
...
**We can then derive apples found:**
   ...
   $$\text{share apples found}= 1-\left(\underbrace{\lambda e^{-p(t_H+t_A)}}_{\text{agent+human search}}+\underbrace{(1-\lambda)e^{-p t_H}}_{\text{human-only search}}\right).$$
...
- For $p=0.5$ and $\lambda=0.5$, we can see the share implied by this expression in a small heat map:
