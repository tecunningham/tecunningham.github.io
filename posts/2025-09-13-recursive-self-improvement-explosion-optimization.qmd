...
2. TO DO:
    - A clearer distinction between two stages: (1) now->R&D automation; (2) automation->singularity.
    - A single canonical model upfront, & then the review can talk about departures from that model.
...
: 
    ...
    We have reason to believe that capability progress is fairly sensitive to the quantity of R&D input. We can quantify the effect with $r$, representing the .  ($r\approx 1$). However there is a great deal of uncertainty on how capabilities will affect R&D.
    
    We can break the effects into two types, and make some very tentative guesses as of April 2026:[^ryan]

    1. Augmentation: agents are accelerating researcher productivity by 50%.
    2. Automation: agents are able to make autonomous contributions equivalent to around 1 month of a researcher's work.
