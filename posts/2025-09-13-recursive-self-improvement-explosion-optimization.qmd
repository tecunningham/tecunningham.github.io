...
<!-- 
SEE ALSO : # 2026-03-06 toy model of AI stack
 -->
...
\begin{tikzpicture}
  \graph [layered layout, grow=right, level distance=3cm] {
    labor -> algorithms,
    "R\&D compute" -> algorithms,
    algorithms -> intelligence,
    "training compute" -> intelligence,
    data -> intelligence,
    intelligence ->["??"] "economic value",
    intelligence ->["??"] "R\&D",
  };
\end{tikzpicture}
```
...
$$
    \xymatrix{
  \text{labor} \ar[dr] & & \text{training compute} \ar[d] & & \text{economic value} \\
  & \text{algorithms} \ar[r] & \text{intelligence} \ar[urr]|{??} \ar[ull]|{??} & & \\
  \text{R\&D compute} \ar[ur] & & \text{data} \ar[u] & & 
}
$$
...
Karpathy (2026): [tweet](https://x.com/karpathy/status/2017703360393318587?s=20): 2.5X/year cost reduction.
: 
    > "This is a 600X cost reduction over 7 years, i.e. the cost to train GPT-2 is falling approximately 2.5X every year. 

@gundlach2026darkenergy "Measuring the Dark Energy of AI Progress": 1.6X/year FLOP-efficiency
: 

    From 2024 to March 2026, for NanoGPT speedrun:
    - 28x FLOP reduction
    - 1.1X non-FLOP reduction
    - 30.7X wall clock reduction

    Would be useful to fill this out for both Hans & Parker:

    |              | 2019-2024 | 2024-2026 | 2019-2026 |
    | ------------ | --------- | --------- | --------- |
    | seconds      |           |           |           |
    | FLOPs        |           |           |           |
    | data         |           |           |           |
...
\begin{tikzpicture}[node distance=0.5cm and 1.5cm,every path/.style={->},every node/.style={draw, rectangle, inner sep=4pt, outer sep=2pt}]
      ...
      \node[ebox] (algorithms) [below right=of labor] {0.5\nodepart{second}algorithms (3X)};
