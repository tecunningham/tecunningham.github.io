...
3. Most models of RSI, as far as I can tell, 
...
::: {.column}
   ...
   1. @gundlach2025algorithmicprogressai argue that the true technology curves are steeper, and shifting left much more slowly. Thus algorithmic progress is relatively less important, and most progress is coming from pure scale.
   2. Epoch note that training compute may not be growing that rapidly.
   3. The technology curve includes anything that maps training expenditure to intelligence: progress in compute cost, algorithms, accumulation of training data, and progress in elicitation.
   4. Labs are probably spending relatively more on R&D (shifting the blue curve left/up) than on training (the black points). 
   
   An important note: there's no consensus on how to measure the y-axis ("model intelligence"). For many applications it doesn't matter as long as we have an ordinal measure (i.e. we can reliably rank model intelligence). 
...
Q: what determines relative expenditure on experiments vs workers?
:   
    - _Random landscape → more experiments._
    - _Regular landscape → more experiments._ Some landscapes are known to be regular, so you can just set a well-known optimization algorithm against them. E.g. hyperparameter tuning, oil discovery.
    - _Expensive experiments → ambiguous._ Even if experiments were cheap, could spend a lot on them. Even if experiments expensive, could spend a small amount (moon landing).
...
:   - _Not just determined by the cost of experimenation._ Even if experiments were cheap, you might want to spend a lot on them, because you're bottlenecked on .
    ...
    - _Structured landscape → more thinking._ If the landscape has some .
