...
- Each period of search you have a probability $p$ of *not* finding an apple.
- A human can find apples over $[0,1]$, an agent can only find apples over $[0,\lambda]$.
- After $n_H$ human searches, and $n_A$ agent searches, then 
    $$\text{apples found}= 1-\left((1-\lambda)p^{n_H}+\lambda p^{n_H+n_A}\right).$$

- Implications:
    - Agents asymptote to a lower level.
    - Agent & human effort is subadditive
    - The value of a search doesn't depend just on the share of apples found, it depends on the state (ie. $n_H$ and $n_A$).
    - There's an asymmetry:
        - If you have a lot of agents, one human is very valuable.
        - If you have a lot of humans, one agent is not very valuable.

- For $p=1/4$ and $\lambda=0.5$, we can see the share implied by this expression in a small heat map:
...
lambda <- 0.5
n_vals <- 0:5
apples <- function(n_H, n_A) {
    1 - ((1 - lambda) * p^n_H + lambda * p^(n_H + n_A))
}

apple_share <- outer(
    n_vals,
    n_vals,
    Vectorize(function(n_H, n_A) {
        apples(n_H, n_A)
    })
)
...
rows <- vapply(seq_along(n_vals), function(i) {
    ...
    paste0(
        ...
        "<th style='padding:0.45rem 0.7rem; border:1px solid #d0d7de; background:#f6f8fa;'>n_H = ",
        n_vals[i],
        "</th>",
...
cat(
    ...
    "Heat map of apples found (p = ", p, ", &lambda; = ", lambda, ")",
    "</caption>",
    "<thead>",
    "<tr><th colspan='2' style='border:none; background:transparent;'></th>",
    "<th colspan='", length(n_vals),
    "' style='padding:0.3rem 0.7rem; border:none; background:transparent; text-align:center; font-weight:600;'>agent searches</th></tr>",
...
- Treating $n_H$ and $n_A$ as continuous, the level sets of $\text{apples found}(n_H, n_A)$ look like this:
...
apple_surface <- outer(
    n_A_grid,
    n_H_grid,
    Vectorize(function(n_A, n_H) apples(n_H, n_A))
)
...
filled.contour(
    ...
    xlab = "agent searches",
    ylab = "human searches",
    main = bquote("Isovalues of apples found" ~ "(" * p == .(p) * "," ~ lambda == .(lambda) * ")"),
...
- Holding one search type at zero, we can compare $\text{apples}(n_H,0)$ and $\text{apples}(0,n_A)$ on the same axes:
...
plot(
    ...
    col = "#1b9e77",
    ylim = c(0, 1),
    xlab = "continuous searches",
    ylab = "share of apples found",
...
legend(
    "bottomright",
    legend = c("apples(n_H, 0)", "apples(0, n_A)"),
    col = c("#1b9e77", "#d95f02"),
    lwd = 2,
    bty = "n"
)
