...
OK here's a very simple argument & model.

|                         | Old AI (shares knowledge) | New AI (creates knowldge) |
| ----------------------- | ------------------------- | ------------------------- |
|                         |                           |                           |
| Superhuman performance? | Only in special cases     | Often                     |
| Who uses it?            | followers in an industry  |                           |


1. It's useful to distinguish between two types of LLMs:
   - **Old LLMs share existing knowledge**-- they are trained on human-produced and human-judged data.
   - **New LLMs discover new knowledge** -- they are trained on the real world.

2. Old LLMs share existing knowledge, & this has some basic economic implications:
   - They're mostly used by firms that are *followers* to catch up to the frontier, or used by people outside their area of expertise.
   - There are rapidly diminishing returns to LLM use: you quickly catch up to the knowledge frontier. Thus adoption is very broad but shallow.
   - They decrease knowledge rents -- people and firms whose value is from their existing knowledge.
...
3. New LLMs discover new knowledge. This has distinct implications:
...
4. A simple model with old & new recipes.
   - Your cost to produce a good is c(r), determined by your recipe r from a set of possible recipes R.
   - Each person knows some set of recipes, and that person's cost is the minimum cost among the recipes that they know of.
   - In equilibrium the firm with the lowest-cost recipe sells the good, & their margins are equal to the difference to the next-lowest-cost recipe.
   - An old LLM makes the existing set of privately-known recipes public, and so all margins go to zero.
   - A new LLM discovers new recipes (but you have to spend on inference).
   - The rents earned by new recipes is due to their scarcity, so it's better to sell the recipes just to one firm.

5. Some concrete examples. You can use new LLMs to:
   - Create a movie
   - Predict stock prices
   - Optimize algorithms
   - Optimize technology
