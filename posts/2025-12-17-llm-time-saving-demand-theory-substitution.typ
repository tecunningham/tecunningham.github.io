...
- #strong["X% saving"] means task-2 productivity increases so that time-per-unit falls by factor $(1 - X)$, i.e., $beta = 1 \/ (1 - X)$. So 10% saving → $beta = 1.11$; 50% saving → $beta = 2$; 80% saving → $beta = 5$.
  ...
  - #strong[Output gain] from the CES formula: $ frac(y', y) = ((1 - s_0) + s_0 thin beta^(epsilon - 1))^(1 \/ (epsilon - 1)) $ where $s_0$ is the #emph[ex-ante] share.
  - #strong[Time savings] = $1 - (y ' \/ y)^(- 1)$, i.e., the fraction of time saved to produce the same output.
...
Let $s equiv t_2^(\*)$ denote the optimal time share on task 2 (and $1 - s = t_1^(\*)$). Express all effects as log-changes $Delta ln y^(\*) = ln #h(-1em) #scale(x: 120%, y: 120%)[\(] y^(\*') \/ y^(\*) #scale(x: 120%, y: 120%)[\)]$ when task-2 productivity moves from $A_2$ to $A_2' = beta A_2$. The last column plugs in $s = 0.1$ and $beta = 2$.
...
#strong[Proposition 4 (finite productivity change on task 2).] If $A_2' = beta A_2$ with $beta > 0$, then $ y^(\*') / y^(\*) = (t_1^(\*) + (1 - t_1^(\*)) beta^(epsilon - 1))^(frac(1, epsilon - 1)) . $

#emph[Proof (explicit)]

+ Replace $A_2$ by $beta A_2$ in $y^(\*)$ from Proposition 2: $ y^(\*') = #scale(x: 180%, y: 180%)[\(] alpha^epsilon A_1^(epsilon - 1) + (1 - alpha)^epsilon (beta A_2)^(epsilon - 1) #scale(x: 180%, y: 180%)[\)]^(frac(1, epsilon - 1)) . $
...
- Cobb--Douglas ($epsilon arrow.r 1$): $y^(\*') / y^(\*) arrow.r beta^(1 - alpha)$ and $t_i^(\*)$ is unchanged.
- Perfect complements ($epsilon arrow.r 0$): $y^(\*') / y^(\*) arrow.r frac(1, t_1^(\*) + t_2^(\*) \/ beta)$.
- Perfect substitutes ($epsilon arrow.r oo$): $y^(\*') / y^(\*) arrow.r beta$ with $t_2^(\*) arrow.r 1$ if $beta A_2 > A_1$.
...
#strong[Connection to our setup:] that's precisely the $epsilon arrow.r 0$ (Leontief / perfect complements) limit in Proposition 5: $ y^(\*') / y^(\*) arrow.r frac(1, (1 - s) + s \/ beta) . $
]
