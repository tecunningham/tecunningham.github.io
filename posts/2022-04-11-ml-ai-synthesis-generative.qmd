...
**The key thing: computers finally are perceiving the latent structure of the world.**

<!--      Key: latent structure in the world, human perceive it (noumena), now we have taught computers to perceive it ; means-ends reasoning is separate ; 
- Human latent structure is only intermediate step , will get deeper one
- Continuous w history, but most latent structure was figured out in 19th century ; 
- Unreliable narrator: novelist knows more than their characters, characters couldn't judge themselves ; 
 -->
...
: They might find patterns that humans haven't discovered yet: a way of discriminating between male and female chicks based on appearance, an association between behaviour and disease (c.f. thalidomide, asbestos, smoking). More broadly a lot of human intellectual history could be thought of as finding latent structures (the cycles of the planets, the laws of physics, the structure of evolution), and computers might discover other deep patterns that we have not yet figured out ourselves.
...
:  LLMs have been very widely adopted by computer programmers, with two obvious uses (1) as a reference, supplying the programmer with the correct function-name; (2) expanding a short prompt into a page of code.

      The second use seems somewhat paradoxical: if a short prompt can be reliably translated into a long code block it implies that the generated code must contain redundant information. This would seem to imply that our programming languages are poorly designed, if it takes a page of code to specify information that could be conveyed in a sentence of natural language.

      I think a good explanation is that there is an asymmetry between writing and reading code: we want to write code with the details implicit but read code with the details explicit. From a natural language description we can get an immediate intuitive understanding of a logical operation, which is sufficient for most cases, but our intuition is not perfectly reliable and so it is important to be able to comb through explicit code to confirm all the details, especially when we are debugging.[^djikstra]

   [^djikstra]: Djikstra wrote an essay, [On the foolishness of "natural language programming"](https://www.cs.utexas.edu/users/EWD/transcriptions/EWD06xx/EWD667.html), arguing that progress in mathematics depended critically on inventing  notations which ruled out the ambiguities of intuition and verbal reasoning:
      ...
      Djikstra is cheering on the tortoise but I think doesn't give appropriate credit to the hare. In many areas it has taken thousands of years of intellectual work for formal reasoning to catch up with intuition -- e.g. to formalize physics, logic, syntax.
...
**2023-02-01: [Science fiction magazine stops submissions because of ML submissions](https://www.pcgamer.com/major-sci-fi-magazine-halts-submissions-due-to-flood-of-stories-written-by-ai-chatbots/)**
...
##              Appendix: We Created AI by Accident [from Facebook]

<!-- I posted this on Facebook: https://www.facebook.com/tom.cunningham.374549/posts/pfbid02wdTFvcBbsJFEU9Fy8B7rdYLrhBbeyNBoUAuAKvLMSS8TUdwVTn4mUNXPDezpRf69l -->
