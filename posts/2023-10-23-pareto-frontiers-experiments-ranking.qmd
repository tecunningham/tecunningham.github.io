...
In this section I generate a bunch of pairs $(X,Y)$ from different joint distribution, and then draw the Pareto frontiers. I don't have anything conclusive to say except that broadly speaking the Pareto frontiers seem to me typically egg-shaped, i.e. not-too-far from ellipses.
...
```{r}
z <- rLaplace(20, mu = 0, b = 3)
Sigma <- matrix(c(1, 0, 0, 1), 2, 2) # 2x2 covariance matrix
points <- data.frame(mvrnorm(n = 20, rep(0, 2), Sigma)) %>%
   rename("x" = 1, "y" = 2) %>%
   cbind(z) %>%
