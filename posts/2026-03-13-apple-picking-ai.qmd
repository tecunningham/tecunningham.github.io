...
: 
    ...
    - Models of discovery.^[@CarnehlSchneider2025, @agrawal2019needles] The apple-picking model models the discovery landscape in a different way (as far as I'm aware) which gives very simple clear results.
...
We can embed the apple-picking model into the workhorse R&D function from @jones1995rd as follows, with the following implications:

1. Human effort has constant elasticity (from a human-only starting point).
2. Agent effort has *declining* elasticity.
...
We can compare this to our apple-picking function, with only human effort $x$:
   $$\ut{a(x)}{apples} = 1-e^{-rx}$$

These can be reconciled if we assume ideas is a nonlinear function of apples-picked ($a$):
   $$A(a)=\ln\left(\frac{1}{1-a(x)}\right)^{1/\beta}.$$

We can then consider progress in ideas with both humans and robots picking apples, $a(x_H,x_L)$:
   $$A(a)=\left[\ln\left(\frac{1}{\lambda e^{-r_Hx_H-r_Ax_A}+(1-\lambda)e^{-r_Hx_H}}\right)\right]^{1/\beta}.$$