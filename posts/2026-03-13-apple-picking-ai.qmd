...
: 
    In this note we describe a model of AI R&D, to help reconcile two observations:
    ...
    The model is somewhat reassuring: agents can be useful for autonomous R&D, without being ready to replace humans. We are not confident that the reassuring implication is correct, but we find the model helpful for thinking through the evidence we would need.
    ...
    The plots below show two effects we see from this model: (1) tortoise-hare dynamics, the agent will make quicker progress than the human at first, but eventually get overtaken; (2) standing-on-shoulders-of-giants: the agent will be able to pick more apples (or optimize the algorithm further) if they start from a stronger human starting point.
    ...
    1. Models of tasks (@acemoglu2022tasks) or knowledge hierarchies (@garicano2006organization). The apple-picking model differs in that tasks are performed *cumulatively*, instead of a fresh set of tasks each period.
    2. Models of knowledge accumulation (@jones1995rd). The apple-picking model differs in that the state of knowledge is not represented by a single scalar: it distinguishes between low-hanging and high-hanging apples.
...
Implications from apple-picking.
:     
    1. **Agents could autonomously improve on human baselines, without replacing humans.** Observing an agent autonomously improve the frontier, finding optimizations that humans have not yet found, does not imply that agents can replace humans, because there are still optimizations that agents cannot find.

    2. **Agent time horizons are not additive.** The model predicts that human time horizons are additive, but agent time horizons are not. Suppose we observe that agents can consistently produce optimizations equal to those found by a human after 1 month of work.^[We can define the time-horizon by equalizing expenditure between agent and human, or estimating asymptotic agent spend.] We should expect this to hold only if we start from a *human-optimized* baseline. If we start from a machine-optimized baseline, then we expect much lower returns (but the opposite doesn't hold, humans are more versatile). Thus we expect significantly different time horizons depending on the starting point.

    3. **We should benchmark agent AI R&D ability against frontier problems.** which represent cumulative human effort, and so are relatively picked-clean, instead of against toy problems which represent the effort of a single human.

    4. **The effects of AI R&D are likely show up in parts of the AI stack that are disproprionately easy optimizations.** e.g. where there's a lot of fairly natural ideas to try but we are bottlenecked on execution.
