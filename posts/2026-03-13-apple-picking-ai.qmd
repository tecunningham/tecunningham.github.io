...
2. **It is critical to understand how agent contribution *scales*.** The two standard models are (A) AI speeds up humans; (B) AI replaces humans. The apple-picking model is one way of explaining: (A) AI can autonomously push the frontier, but hit diminishing returns; (B) agents and human expenditure are complements.
...
Consider the history of AI R&D.
: 
...
Simple models imply a change in slope.
: 
    There are two simple models of AI's effect on R&D: (1) it multiplies the effectiveness of human researchers; (2) it replaces human researchers with computer researchers. Both models imply we should the historical pattern of expenditure-progress to stay on the same path, but accelerated:
    ...
    \begin{tikzpicture}
        ...
        \draw[red,thick,domain=4:4.2]
            plot (\x, {3*exp(-\rH*4)*exp(-4*(\x-4))}) circle (0.03) node[right] {agent contribution};
    \end{tikzpicture}
    ```

Agents can improve on human baselines without being perfect substitutes for humans.
: 
    The apple-picking model implies a different pattern: agents will autonomously advance the frontier, but they will then quickly hit diminishing returns. When the marginal returns to human and agent expenditure are equalized, then we will return to investment in more human optimization:
    ...
    \begin{tikzpicture}
        ...
        \draw[green!50!black, thick, domain=0:4, samples=120]
            ...
            circle (0.03);
        %\draw (0,.7)--(5,.7);
        %\draw (0,{3*exp(-\rH*4)})--(5,{3*exp(-\rH*4)});
        \draw[red,thick,domain=4:4.2]
            plot (\x, {.7+(3*exp(-\rH*4)-.7)*exp(-30*(\x-4))}) circle (0.03) node[right] {agent contribution};
...
: 
    ...
    \pgfmathsetmacro{\rightTreeX}{\leftTreeX+\treeSpacing}
    \pgfmathsetmacro{\groundLeft}{-5}
    \pgfmathsetmacro{\groundRight}{20}
    \pgfmathsetmacro{\rowStep}{(\canopyTop-\canopyBot-2*\canopyPad)/(\nrows-1)}
