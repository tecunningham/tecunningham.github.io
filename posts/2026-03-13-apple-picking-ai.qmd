...
: 
    ...
    2. The agent will be able to pick more apples if they start from a stronger human starting point (standing on the shoulders of giants)
...
: 
    We discuss below evidence that agents are autonomously able to find improvements on a variety of algorithms that have already been well-optimized by humans.
    
    In the apple-picking model it's possible for agents to advance the frontier, without them being strictly superior to humans. We would expect progress to go as follows: that deploying agents will cause a one-time spike in progress, until the marginal returns to human and agent expenditure are equalized, then a return to human progress:
    ...
    We give quotes below from a variety of domains that the contributions of agents seem to have hit diminishing returns.
...
AI R&D ability should be benchmarked against frontier problems.
: 
    We should use problems which represent cumulative human effort, and so are relatively picked-clean, instead of against toy problems which represent the effort of a single human.
