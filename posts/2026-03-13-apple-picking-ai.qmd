...
1. AI agents are autonomously doing R&D, now what? @slimshetty_ and I give a simple formalization of the low-hanging-fruit metaphor & draw out some implications:  [image]
    ...
    2. Agents will jump ahead of humans but then get overtaken
    3. Agent contribution depends on the starting point

2. These implications are consistent with @karpathy, Nathan Lambert, Terence Tao, who've said that agents have made genuine contributions, but only up to a point.

3. Suppose robots can pick apples at a lower price than humans, but only apples up to a certain height. Then you'll want both robots and humans working on your orchard. If you put them in a race it'll be tortoise vs hare: the robots will do better in the short-run, but eventually humans will pick more apples. These seem consistent with agents doing R&D.
    [image]

4. We'll also see a dependence on initial conditions: robots will leave fewer apples unpicked if they start from a tree that a human has already worked on. This helps to measure the reach of agents.

5. Why do we care? We want to forecast the future path of AI capabilities, & this framework makes it a bit easier to quantify the contribution from (A) AI accelerating human work; (B) AI replacing human work with autonomous agents.
...
: 
    Many people are talking about how AI is autonomously able to contribute to frontier R&D, yet it's only picking low-hanging fruit: [Andrej Karpathy](https://x.com/karpathy/status/2031135152349524125), [Terence Tao](https://www.theatlantic.com/technology/2026/02/ai-math-terrance-tao/686107/), [Nathan Lambert](https://www.interconnects.ai/p/lossy-self-improvement). In this note we describe a model of AI R&D, to help reconcile two observations:

    1. AI agents are making genuine autonomous contributions to R&D (evidence below).
    2. AI agents do not yet seem a perfect substitute for humans researchers.
    ...
    The model has somewhat reassuring implications: agents can be useful for autonomous R&D, without being ready to replace humans. We are not confident that the reassuring implication is correct, but the model is helpful for thinking through the evidence we would need.

    We give a formal model but the basic ideas can all be seen in the drawing below. Here both the human and robot have picked four apples, but they've left the tree in a very different state, such that the robot isn't ready to replace the human yet:
    ...
    We can write the core equation, where $\lambda$ is the height of the robot, $t_H$ and $t_A$ are the time spent by the human and agent, and $r_H$ and $r_A$ are the rates at which they find unpicked apples:

    $$\text{apples remaining}= \underbrace{\lambda e^{-r_Ht_H-r_At_A}}_{\text{apples on bottom}}+\underbrace{(1-\lambda)e^{-r_Ht_H}}_{\text{apples on top}}.$$

    The plots below show two implications of the model:
    
    3. The agent will make quicker progress than the human at first, but eventually get overtaken (tortoise-hare dynamics)
    4. The agent will be able to accumulate more apples (leave fewer unpicked) if they start from a stronger human starting point
...
: 
    ...
    In reality aggregate progress is likely to be smooth for a few reasons: (1) models are released at a quick cycle, and harnesses are constantly being updated; (2) each human discovery opens up room for agent discoveries (assumed away in the model); (3) LLMs are used to augment human activity, as well as autonomously do R&D.


Agent time horizons are not additive.
: 
    Because of the tortoise-hare behavior of agents we can calibrate an agent's ability by the point at which a human and an agent, given equal expenditure, will make equal progress. This is, very loosely, the way time horizon is identified in @wijk2025rebench and @kwa2025longtasks.
    ...
    In the apple-picking model an agent's time horizon will depend on the starting point. If we have an algorithm that has only been optimized by humans, we expect agents can push it forward a lot. But if we have already applied some agent labor to the algorithm, then further agentic labor will have much lower returns, i.e. time horizons will be much shorter. Concretely: one agent can be as good as one human (or human-week), but two agents are not as good as two humans.


A low-hanging apple is whatever is easy for the agent.
: 
    The model is defining an optimization as low-hanging if an agent can do it. In practice many people have made conjectures about what specific types of tasks/ideas are within reach of agents (e.g. well-defined, hill-climbable, short-time-horizon).
...
: 
    ...
    In contrast other parts of the stack can be more easily hill-climbed, cheaply iterating on ideas to find improvements. Note that we should expect the returns to human labor to be roughly equalized across each of these trees, so the relevant question for agentic contribution is just the relative density of low and high apples.
...
: 
    - *Uplift.* In practice AI will both (A) accelerate humans; (B) replace humans. The apple-picking model focuses on the second for clarity.

    - *Directed search.* We assumed that the probability of finding an apple is independent of other apples already found. Realistically people have an ability to direct their attention to finding new innovations.
    
    - *Bottlenecks.* The discussion has abstracted from other bottlenecks, e.g. the necessity for experiment, which are obviously important, see for example @whitfill2025bottlenecks.
...
Setup.
:   There is a continuum of apples spread uniformly on [0,1].
    
    A human can find apples over $[0,1]$, but an agent can only find apples over $[0,\lambda]$, with $\lambda < 1$ (at least for now).
    
    Humans find unpicked apples at rate $r_H$, agents find apples at rate $r_A$, and we use $t_H$ and $t_A$ to represent the time humans and agents spend searching (you can also interpret $t_H$ and $t_A$ as monetary expenditure).
    ...
    \begin{tikzpicture}[
        ...
        \draw[dotted] (\Lx+2,0) -- (\Lx+2,5) node[above]{$e^{-r_Ht_H}$};
        \draw[dotted] (\Lx+3,0) -- (\Lx+3,5) node[above]{$e^{-r_Ht_H}$};
...
% titles
\node[font=\bfseries] at (\Lx+2,7.6) {Human + AI Research};
\node[font=\bfseries] at (\Rx+2,7.6) {Fully Automated Research};

% shared top annotation
%\node[align=center] at (\Lx+2,7.7) {Apples extend to\\arbitrary height};
%\draw[->] (\Lx+2,6.9) -- (\Lx+2,6.2);
%\node[align=center] at (\Rx+2,7.7) {Apples extend to\\arbitrary height};
%\draw[->] (\Rx+2,6.9) -- (\Rx+2,6.2);
...
\node[left] at (\Lx,0) {0};
\node[below] at (\Lx,0) {0};
\node[below,align=center] at (\Lx+2,0) {$h_n$\\human\\progress};
\node[below] at (\Lx+4,0) {1};
%\node at (\Lx+2,-0.9) {Human progress $h_n$};
\node[left, align=right] at (\Lx-0.15,4) {Human\\height = 1};
\node[left, align=right] at (\Lx-0.15,2) {AI height\\$\lambda_n < 1$};
...
% right panel axes / guides
\draw[thick] (\Rx,0) -- (\Rx,6.2);
\draw[thick] (\Rx+4,0) -- (\Rx+4,6.2);
\node[left] at (\Rx,0) {0};
\node[below] at (\Rx,0) {0};
\node[below] at (\Rx+4,0) {1};
\node[left, align=right] at (\Rx-0.15,4) {Human\\height = 1};
\node[left, align=right] at (\Rx-0.15,6) {AI height\\$\lambda_n > 1$};
\draw[dashed] (\Rx,4) -- (\Rx+4,4);

% right panel shaded region
\fill[applefill, opacity=0.75] (\Rx,0) rectangle (\Rx+4,6);
\draw[line width=1.8pt] (\Rx,0) rectangle (\Rx+4,6);
%\node[font=\bfseries] at (\Rx+2,6.5) {unpicked};
\node[text=white, align=center, font=\itshape\small] at (\Rx+2,2.5) {picked by\\agents};

% middle annotation
\node[align=center] at (6,2.0) {$a_n :=$ total\\picked apples};
\draw[->] (5.0,1.9) -- (4.35,1.9);
\draw[->] (7,1.9) -- (7.65,1.9);
\end{tikzpicture}
```
...
Assumptions.
: 
    - Apples are distributed uniformly between 0 and some arbitrarily large value.
...
\begin{aligned}
    \utt{s_i}{share remaining}{from height $i$} &= 
        \begin{cases}
            1 &, \lambda^H < i & \text{(beyond human reach)}\\
            e^{-x^Hr^H} &, \lambda^A < i< \lambda^H & \text{(human picked)}\\
            e^{-x^Ar^A-x^Hr^H} &, i<\lambda^A & \text{(human and agent picked)}
        \end{cases}
\end{aligned}
...
\begin{aligned}
    \utt{s_{i,t}}{share remaining}{height $i$, time $t$} &= 
        \begin{cases}
            1 &, \lambda^H_t, \lambda^A_t < i & \text{(beyond both reach)}\\
            e^{-tx^Hr^H} &, \lambda^A < i< \lambda^H & \text{(human picked)}\\
            0 &, \lambda^H < i< \lambda^A & \text{(agent cleared)}\\
            0 &, i<\lambda^A,\lambda^H & \text{(agent cleared)}
        \end{cases}
\end{aligned}