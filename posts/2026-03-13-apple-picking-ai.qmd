...
1. AI agents are autonomously doing R&D, now what? @slimshetty_ and I give a simple formalization of the low-hanging-fruit metaphor & draw out some implications:  [image]
    ...
    3. The relative value of an agent depends on how much you've used agents so far.
...
: 
    ...
    The model implies that agents can push the frontier forward, but the returns will sharply diminish. We are not confident that the reassuring implication is correct, but the model is helpful for thinking through the evidence we would need.
...
: 
    ...
    Note that the distribution of apples could be non-uniform, we would just need to substitute $F(\lambda)$ for $\lambda$ in the expression for apples remaining.
...
: 
    ...
    $$\lambda_{t+1}(a_t) = \beta \min\{0,a_t-\bar{a}\}.$$
