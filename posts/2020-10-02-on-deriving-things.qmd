...
<!-- TO ADD: (1) i'm stitching intently on a quilt and when I look up I see that the quilt covers the bed, goes out the window, ; (2) my daughter suckling on a bottle -->
...
![](images/2024-11-29-07-13-26.png)
...
: I don't want to get in but once I'm in I don't want to get out.
      ...
      When daughter was a baby she would push away a bottle of warm milk until the nipple was in her lips and then she'd clutch it and drink deeply.

<!-- , once the patient is on the operating table and the chest is cut open, -->
...
: By the third week I was still going down to the beach, getting in the water, and starting to swim, but after 30 seconds I would let my legs fall to the sand and just walk through the water staring into space and thinking. Then I would remember to start swimming again.
...
[How to interpret experiments with two outcomes](https://joshkim.org/files/InterpretingExperiments.pdf)
: Your estimate of the treatment effect on one outcome will depend on the observed effect on the other outcome, and the weight will be proportional to the difference in covariances between the treatment effects and the noise.

[When tradeoff curves look like ellipses](https://tecunningham.github.io/posts/2023-10-23-pareto-frontiers-experiments-ranking.html)
: If you're ranking items by predictions, e.g. ranking by $p(A)+\beta p(B)$, and the probabilities have a joint Gaussian distribution, then the overall tradeoff between $A$ and $B$ will be an ellipse, equal to an isovalue of the joint distribution of $F(p(A),p(B))$.
...
: If information is dispersed in the brain, and aggregated sequentially, then mistakes will occur when there are interactions (nonseparabilities), and there are certain characterisitic inconsistencies which will reveal the nonseparabilities.

[Implicit preferences can be inferred from choices.](https://jondequidt.com/pdfs/paper_implicit.pdf) (with Jon de Quidt)
: Observing choices over outcomes which differ in attributes can reveal the difference between implicit and explicit preferences -- and implicit prefernces can be due to tacit knowledge, signaling motives, or constraints on allowable decisions.
...
:  Suppose you'll admitting a student to your PhD program only if they have a GRE score that implies above-average abililty. If students can exert effort to inflate their scores then you'll end up admitting more than half of the students, even if you rationally adjust for the inflation.
...
- I knew that (B) is not true for all $\mu$, but I knew that it's true for sufficiently small values, i.e. $|\mu|<k$ for some $k$.

- So I had two versions of an assumption ($\mu$ is zero, $\mu$ is small), and two versions of (A).

- When my mind wandered onto whether claim (A strong) was justified I was reassured by remembering that it'll work when $\mu=0$ (true for both claim A and B), and additionally that for $\mu=\epsilon$ it'll be OK (true only of claim B).
