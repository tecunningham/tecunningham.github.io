...
##             Hardy et al. (2015) "Strategic Classification"
...
#           Model of Latent Space

**Summary:**

1. Humans observe a high-dimensional message $\bm{x}$, and we infer from that a low-dimensional semantics $\bm{v}$. We can also do the reverse: given some semantics $\bm{v}$ we generate a message $\bm{x}$.
2. We can train a classifier, feeding it examples of $\bm{x}$ and $\bm{v}$, but it'll do badly, the feature space is far higher than the number of samples.
...
**Formal setup:**
   ...
   $$\begin{aligned}
      ...
      v  &\sim N(0,\Omega)\\
      e  &\sim N(0,\Sigma)\\
      E[v|x]   &\simeq A\Sigma(A\Sigma A'+\Sigma)^{-1}x
...
**Two dimensional example.** 
   ...
   - _Simple classifier is easy to evade:_ An adversary can just lower their $x_1$ and raise their $x_2$ such that they get the same value of $v$ but fall outside the classified region.
