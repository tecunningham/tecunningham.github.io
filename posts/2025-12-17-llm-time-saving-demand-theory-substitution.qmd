...
:   1. The total **output gain** will be between $\frac{1}{(1-s)+s/\beta}$ (if perfect complements/Amdahl) and $\beta$ (perfect substitutes).
    ...
    2. If the time-savings are tiny then Hulten's theorem applies: $\Delta\ln y \approx s\ln\beta$ (and for tiny changes, time-saved $\approx \Delta\ln y$ too).
    
    3. If the time-savings are small then you can use elasticity of substitution (there's a closed-form expression but it's ugly).
