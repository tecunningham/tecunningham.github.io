...
Over the last 50 years there have been steady improvements in computer interpretation of text and images, and especially since around 2012 (the introduction of deep neural nets), now often surpassing the performance of an average human. Since around 2018 computers have also become very good at *synthesizing* content, e.g. creating text, images, or video, to match some description.
...
AI models will keep getting better at detecting nudity, obscenity, hate speech, slur words, incitement, illegal acts, violence. The same AI models will also allow producers to obfuscate violating content but as the AI models become arbitrarily close to human judgment then fewer and fewer exceptions will slip through, i.e. fewer items which a human recognizes as violating but the computer does not. Below I give a more formal argument for this ("Venn diagram model"). This prediction applies just to "internal" categories of content where the ground truth is human interpretation without reference to outside information, not to "external" categories such as deepfakes or misinformation, discussed below.

This prediction is consistent with recent trends. For example Meta reports that the prevalence of nudity, bullying, hate speech, and graphic content have each declined over 2017-2022 by a factor of between 2 and 5, and that the share of actioned content that is detected by an ML model ("proactive rate") is approaching 100% for most categories.[^metaDecline]

![](./2023-01-31-social-media-suspensions-data_files/figure-html/unnamed-chunk-28-1.png)

This prediction also applies to censorship. If an authoritarian governments uses crude pattern-matching to detect anti-government language then that can be circumvented by using indirect language. Better AI will allow governments to classify every post with reliability as high as if they had a human read each one of them, and so indirect language or obfuscation will no longer be an effective workaround.

This prediction would fail if there were substantial limits on the performance of AI, e.g. if there were ways of obfuscating content so that a human would recognize it but an AI would not. It seems plausible that AI systems will remain less flexible than humans in general but because they are trained on enormous quantities of data, what they miss in the short-run they should be able to quickly learn to recognize.
...
Whether prevalence will go precisely to zero depends on the definition of prevalence used. An AI model can never perfectly predict human-rater evaluation because there will still exist significant between-rater and within-rater variation in labelling for any given piece of content.
...
Prevalence of policy-violating content seems likely to decline. However we should expect substitution towards content that violates the policy's spirit without violating the letter.

I will use the phrase "context-specific violations" to refer to posts that are interpreted by most of their viewers to have a significance that is violating, but interpreted by professional raters (who are not given any information about context) as having a non-violating significance. Some examples:[^selective]
...
- Using photos without nudity but which can be read as pornographic.[^selective] 
...
It is possible that platforms will try to catch more context-specific violations: raters could be given more detail about the context and the prior history of the post's author, and raters could be assigned to posts depending on their experience with the argot of the community. Likewise AI models could be trained to include rich representation of the context.^[Platforms already have some policies that include context, e.g. Facebook's ["Bullying and Harassment policy"](https://transparency.fb.com/policies/community-standards/bullying-harassment/)  bans "repeatedly contacting someone in a manner that is unwanted or sexually harassing."] An additional advantage of adding context is that it can identify and exempt posts that violate the letter but not the spirit of the policy.
