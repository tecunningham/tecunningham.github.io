...
**It is difficult to anticipate the effect of AI on communication.** AI models have already had big effects on content moderation (detecting spam, bots, policy-violating content), but they are also starting to have big effects on content production (obfuscation, generating synthetic spam, deepfakes).

**In this note I try to make some predictions about the equilibrium impact of AI.** Briefly:
...
![](http://localhost:3471/posts/2023-01-31-social-media-suspensions-data_files/figure-html/unnamed-chunk-28-1.png)
...
##          (2) Context-specific violations will increase
...
I will use the phrase "context-specific violations" to refer to posts that are interpreted by most of their viewers to have a significance that is violating, but interpreted by professional raters (who are not given any information about context) as having a non-violating significance. Some examples:
...
It is unclear to me how much appetite platforms will have for adding context to their evaluation process so that they can also identify context-specific violations. It is not technically difficult: raters could be given more detail about the context of the post and the prior history of the post's author, and raters could be assigned to posts depending on their experience with the argot of that post's audience. Likewise AI models could be trained to include details about the context.

If platforms continue to judge violations in a context-free way then we might expect an increase in context-specific violations. It is hard to anticipate whether platforms will wish to extend their definition to include context-specific violations. Authoritarian governments are willing to punish speech based on the effect (pragmatics) rather than just the semantics, e.g. in 2022 Hong Kong police arrested protestors for holding blank pieces of paper. However platforms might wish to .

where violation is judged by human evaluators.

The prior discussion applied to the typical interpretation of policies as "context free", meaning that the violation is judged without reference to the audience or prior history of the author.

We can distinguish between "context-free" and "context-specific" violations, e.g. whether a post would be interpreted as denigrating an ethnic group by any observer, or only by an observer who is member of a subcommunity or .

Platforms typically measure policy violations in a "context-free" way: they send a post to a human who makes a determination without information about who is seeing the post, the context in which the post appears, or the author of the post.

Thus it is possible for a post to be evaluated as non-violating but *in context* to be interpreted as having a policy-violating significance. For example using double entendres, substituting "globalist" for "jewish", saying the opposite of what is meant (e.g. saying excessively positive things about a hated ethnic group, or a pro-anorexia community speaking as if they were anti-anorexia), using euphemisms for illegal acts, using emojis of eggplants and peaches, using photos without nudity but which can be read as pornographic.[^selective] 
