...
Prevalence of policy-violating content seems likely to decline. How ever we should expect platform users to substite towards producing content that achieves their intent (communicating a violating message) without being flagged as violating (having a human rater recognize the message as violating). This can be achieved with "context-specific violations": posts that are interpreted by the intended viewers to have a significance that is violating, but interpreted by professional raters (who are not given any information about context) as having a non-violating significance.[^definition] Some examples:[^selective]

- Saying "globalist" when your audience understands it to mean "jewish"
- Saying the opposite of what is meant, e.g. a bigot saying excessively positive things about a hated ethnic group, or a pro-anorexia poster making sarcastic anti-anorexic statements.
- Using euphemisms for illegal substances or illegal acts.
- Using emojis of eggplants and peaches.
- Using photos without nudity but which can be read as pornographic.
...
It is difficult to make generalizations about overall human persuadability, but here are some broad attempts:

1. Human behaviour is very sensitive to influences. Adult express

2. Each decade of the 20th century has had some prophets warning of the new technologies of human manipulation: using subcoscious associations (Bernays), using conditioning (Skinner), using subliminal messages (Packard), using brainwashing (Sargant), using propaganda techniques of Fascism, using the power of conformity (Adorno, Asch, Zimbardo), using priming and nudges (Bargh, Thaler), or using statistical profiling (Cambridge Analytica).

3. Carefully measured evidence of persuasive effects tend to be low. 

4. Nevertheless 1/3 of the economy is devoted to persuasion, and attitudes and preferences vary between populations primarily due to social context implying there must be *some* extra-rational means by which people form their attitudes. -->


##          Influence Operations will not be substantially affected

A 2022 [report](https://arxiv.org/pdf/2301.04246.pdf) from the Stanford Internet Observatory lists a number of ways in which LLMs could be used by influence operations, mainly by writing copy either for social media posts or for 1:1 message threads. There does not seem to be evidence that the current generation of LLM-created messages would be more persuasive than human-created messages.[^LLMpersuasion] Thus the effect on Influence Operations will be to lower the cost of content production, however I'm not sure this will have a dramatic effect on their overall effectiveness because it's not clear that they're primarily constrained on the volume of text that they can produce, influence operations also are constrained on creating accounts, compromising existing accounts, building audiences, and monitoring and experimenting with different messages.
