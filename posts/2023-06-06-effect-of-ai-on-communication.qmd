...
::: {.column-margin}
   ...
   Thanks to many comments, esp. Ravi Iyer, Sahar Massachi, Tal Yarkoni, and Rafael Burde.
:::
...
**This note discusses a range of evidence and theory, summarized with a set of predictions:**

1. **The prevalence of policy-violating content on platforms will decline.** E.g. the share of nudity, hate speech, and harassment. Classifiers are already approaching human-level performance, which is the ground truth, and so prevalence should approach zero. The same argument applies to censorship.
...
3. **The prevalence of variations of known-violating content will decrease.** E.g. content that is a match against databases of illegal sexual media (PhotoDNA), IP-protected content (ContentID), or terrorist recruitment content (GIFCT). Obfuscation will become harder as AI models get better.
...
5. **People will not be able to discriminate between real and fake media based only on the content.** Until now it has been often possible for people to detect whether recorded evidence was manipulated (photo, audio, video). That will no longer be true.
6. **Platforms will not be able to discriminate between real and fake media based only on the content.** It is unlikely that AI classifiers will be able to detect AI-manipulated media. This doesn't change the status quo: historicall misinformation classifiers are very poor if they use only content features.
...
8. **Computer-generated content will not be hyper-persuasive.** LLMs are unlikely to find the hidden key for manipulating public opinion. LLMs imitate what other people say, they have very limited insight into how it works. Social scientists have warned about hyper-persuasion for a century and been wrong every time.
...
Finally the notes has short appendices on the history of forged documents and on relevant forecasting questions from Metaculus.
...
##          The prevalence of database-matched violating content on platforms will approach zero
...
Most online platforms struggle with automated users (bots) who are disruptive in a variety of ways. One way of protecting against bots is with behavioral tests, e.g. a CAPTCHA test asking users an image-recognition task[^captcha], or by using on-platform behavior to detect whether a user is human. However improvements in AI mean that computers can now recognize most images as well as humans, and can learn to imitate human-style behavior patterns, so it seems likely these behavioral tests will become ineffective against sophisticated actors.[^fingerprinting]
...
<!-- 
##          Computer-Generated Content will not be Hyper-Persuasive
...
[^LLMpersuasion]: This is also my interpretation of Goldstein et al. (2023) ["Can AI Write Persuasive Propaganda"](https://osf.io/preprints/socarxiv/fp87b/): they use GPT-3 to write persuasive text and get humans to rate its persuasiveness. Under some narrow circumstances they find GPT-3's text is as persuasive as human-written text.
...
\begin{tikzpicture}[scale=2]
   ...
   \draw[black] (-2,0) -- plot[domain=-2:2] (\x,{exp(-(\x)^2)}) -- (2,0) -- cycle;
   \node at (0,1.2) {real};

   \draw[red] (-2,0) -- plot[domain=-2:2] (\x,{exp(-(\x+1.5)^2)}) -- (2,0) -- cycle;
   \node at (-1.5,1.2) {fake};

   \draw[dashed] (-.75,0) -- (-.75,1) node[above] {blah};
