...
This table summarizes some basic empirical facts about social media. I put an asterisk ( * ) next to entries where I am *not* aware of a good reference, specifically a reference which discusses the balance of evidence. However the discussion below reflects my personal views on each issue, and I sometimes is not very heavily cited.

+-----------------+---------------------------------------------------+----------------------------------------+
|                 | empirical facts                                   | causal effects                         |
+-----------------+---------------------------------------------------+----------------------------------------+
| platform usage  | - usage across platforms                          | - substitutability (diversion ratio)   |
|                 | - overlap* (AKA multi-homing)                     | - marginal network effects*            |
|                 | - clustering* (country/city/demographic)          | - speed of adjustment*                 |
|                 | - typical lifecycle of social networks            |                                        |
|                 |                                                   |                                        |
+-----------------+---------------------------------------------------+----------------------------------------+
| business/profit | - ad load (by platform type)                      | - user ad-load elasticity              |
|                 | - price (by platform type)                        | - elasticity in demand for ads         |
...
|                 |                                                   |                                        |
+-----------------+---------------------------------------------------+----------------------------------------+
| politics        | - correlation with populist politics              | - effect on polarization               |
|                 | - correlation with polarization                   | - effect on coordination/protest       |
|                 | - correlation with protest & revolution           | - effect of censorship on politics     |
|                 | - breadth of censorship                           | - effect of influence operations       |
...
**Dynamics.** Social network markets tend to be dominated by the first mover, and are rarely replaced. Most change is between-cohort, not within-cohort.

**Causal.**

   - _What determines demand for social media?_ Extremely high dimensional but want simple first-order approximations. Simplest model has `p` as share of population active ("reach"), and equilibrium where `p=f(p)`. This gives you extensive and intensive margin, tipping point, network multiplier. Then can add extra parameters for richer models: speed of adjustment, degree of substitutability. (We know demand is 100% network effects but need to figure out how steeply they diminish in different directions (friends/family/geographically/interest-based), and how they interact with quality.)
   ...
   - _How substitutable is social media with other uses of time?_ Hard to estimate substituability separately from network effects; longitudinally seems perhaps 50% of social media growth has been at expense of television.

   - _How big are network multipliers._ Very little clean identification: cluster experiments are almost always under-powered, outages have little external validity. But can estimate from observational distribution.

   - _Causal effect of deprivation._ Many studies on WTA get broadly consistent answers. Few good deprivation studies, results tend to be weak and hard to interpret because of dynamics and network effects. Addictive effects seem real but small (comparable to television or radio?). Observational effects on mental health, esp teens (I haven't been following this literature).
