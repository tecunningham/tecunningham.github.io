...
: We want to predict the effect of AI on dozens of different areas of life -- on medicine, on law, on entertainment, on news, on competition and markups, on scientific discovery, on personal relationships. It seems to me that there is a lack of unified theories which could be applicable to all these domains.
      ...
      In this section I will complain a little about the state of theorizing about the economic effects of AI. I feel that there's not enough ambitious work which tries to model the effects of AI on human society in a deep structural way, which would allow us to make predictions across many domains.

      There's a good chance that I'm misinterpreting or missing important things, please tell me what I've missed.

The field still seems a little inchoate.

: It seems to me that the field of economics-of-AI is still pretty open:

      1. There is no commonly accepted definition of Artificial General Intelligence, or Superintelligence, or Transformative AI.
   
      2. There is no standard way of modelling AI in economics, e.g. most papers in the NBER workshop used different assumptions. There are relatively few classic models in the economics of AI.
...
: Here are two classes of somewhat more structural models. 
      ...
      1. @agrawal2019predictionjudgmentcomplexity -- here AI improves the ability to predict some outcome, & the authors argue this is typically a complement to human judgment about preferences. A couple of notable features: (1) in this model AI and human intelligence are qualitatively different, they assume that only humans can make preference decisions, which I find hard to interpret (human decision-making in paid work is by definition instrumental, you are adopting someone else's preferences); (2) the model doesn't seem to naturally predict the knowledge-sharing feature that LLMs seem to have (the model was originally proposed before the rise of generative AI).
...
: Here is a simple mental model that I often use: *LLMs share knowledge*. The model is unsatisfactory in many respects but has the virtues of being very simple and very general. Consider an LLM as just a database of answers to questions, containing the set of answers that already exist in the public domain (i.e., in the LLM's training set).[^blog] LLMs therefore lower the cost of access to existing knowledge, and people will consult an LLM when they encounter a problem for which (i) they do know know the answer, but (ii) they expect that someone else does have know the answer (and the answer was included in the training set).
...
: There has been a lot of work by economists on the limits of human cognition over the last 50 years. This is a natural quarry where we can get material for a theory of AI's economic impact, however I'm not very optimistic.

      Ideally we could start with a model of human judgment and decision-making, and change the parameters to fit a model of AI judgment and decision-making. However there are not many clear candidates, theories in behavioral/psychological economics tend to emphasize biases: prospect theory, hyperbolic discounting, ambiguity aversion, heuristics, social preferences, rational inattention.
      
      These are primarily theories of the *weaknesses* of human judgment, as such they do not seem to offer much explanation of why computers have been worse than humans at most judgment and decision-making for so long (and still are worse at many things).
