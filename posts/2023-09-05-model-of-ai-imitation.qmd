...
<!-- 
   ...
   TODO: add Rob's comments and examples
   TODO: series of cases and illustrations from GPT4 -->

::: {.column-margin}
   ![](20230825131919.png)
   Thanks to comments from many, especially [Giorgio Martini](http://www.giorgiomartini.com), Grady Ward, [Rob Donnelly](https://robdonnelly.me), and [Inés Moreno de Barreda](https://sites.google.com/site/inesmorenodebarreda/).
:::
...
**If we train AIs to *imitate* humans, will they ever *beat* humans?** Large language models (LLMs) are trained to predict what humans would say in a given situation (with some qualifications, see below). It would seem to follow that such a model would never be *better* at answering questions than a human.[^objective]
...
**The question seems important.** AI has caught up to human performance on many benchmarks, largely by imitating humans, it seems important to know whether this is a ceiling or we should expect them to shoot out ahead of us. Will LLMs be able to write superhumanly-persuasive prose? Will image models be able to see things in photos that we cannot? There is a lot of technical literature on imitation learning in AI but I haven't found much discussion of this point.
...
**This blog post contains:** 
   ...
   5. A simple form model with a derivation of each of the five mechanisms.
...
**(4) Priors.**[^rob] When making extrapolation from the same set of data humans and computers will given different answers because they have different priors, and in some cases computer priors might give a better fit.
   ...
   There is an interesting sublety in how the AI ought to be prompted, consider these two questions:
...
**(5) Tacit Knowledge.** 
   ...
   The relative success of machine learning over symbolic AI has often been connected to the importance of tacit human knowledge, e.g. by Geoff Hinton. I have written a lot about the importance of tacit knowledge in human decision-making, especially @cunningham2015hierarchical and this [post](https://tecunningham.github.io/posts/2017-12-10-unconscious-influences.html), see also @cunningham2022implicit. In memory a related phenomenon is the "recognition recall" gap (@macdougall1904recognition): people are significantly better at recognizing whether they saw a word before than in recalling that word.

   In fact I think this is a reasonable description of image synthesis by neural nets: models are first trained to recognize images given captions, and then a new image can be synthesized to match a given caption, e.g. through a diffusion algorithm. Here there's a striking asymmetry: algorithms can approximately match average human performance in recognition, but they far outperform human performance in construction of new artefacts.
...
**Summary.** This table shows the year in which a computer (or mechanical device) could match performance with the best human:
...
**Computers have hit the ceiling on most benchmarks.**  @kiela2023plottingprogress documents that most computer benchmarks have become "saturated," i.e. computers get close-to-perfect performance, and that recently the speed of saturation has become quicker (see graph on right). They say identify only a single benchmark where performance is not close to the human baseline, and most of the models they discuss are imitation learning. As a consequence some work has moved to evaluating models against examples specifically on "adversarial" tasks chosen to fool computers (e.g. Dynabench, @kiela2021dynabench).^[@kiela2021dynabench also say that "models that achieve super-human performance on benchmark tasks (according to the narrow criteria used to define human performance) nonetheless fail on simple challenge examples and falter in real-world scenarios."]
   
**On some benchmarks no computer could ever beat human performance.** On some tasks human performance effectively *is* the ground truth, and so by definition computers could never beat humans. This is roughly true for text comprehension: a sentence has a given meaning if and only if the average person believes it has that meaning. When we observe computer outperformance it is because either (1) there is human variation and the computer output is more consistent; or (2) computers outperform amateur humans but the ground truth is expert humans.
...
***Summarizing text*: computer beats average human.** @lee2023rlaif reports that LLM-generated summaries of articles were preferred by humans to human-generated summaries 80% of the time. However the paper doesn't give much detail about the conditions of the humans writing the summaries.[^summarization]

   [^summarization]: *"RLAIF summaries are preferred over the reference [human-written] summaries 79% of the time, and RLHF are preferred over the reference summaries 80% of the time."*
...
**We can formalize "superhuman" ability in a variety of ways.** The notation is introduced more fully below, but briefly $\bm{q}$ represents a question, $a(\bm{q})$ represents the correct answer, $\bar{a}(\bm{q})$ represents the computer's answer, and $\hat{a}_i(\bm{q})$ represents the answer of human $i$. The expectations can be interpreted as over the universe of all questions, or some subset of questions:
...
**Computers can outperform humans on recognition tasks.** Most benchmarks for media recognition use human labels, e.g. for object detection in photos or speech recognition in audio. However humans can be mistaken: they might incorrectly think a photo has a dog in it when it does not or vice versa. Humans are very good at recognition, so human-labelled data has been sufficient, but computers are liable to exceed human performance on these tasks.[^groundTruth]
   ...
   [^groundTruth]: Defining the ground truth in a recognition task is somewhat complicated because a given set of pixels is consistent with an infinite variety of objects causing that arrangement, we are able to talk about an image representing an object in the context of strong priors about the world. So the ground truth of interest must be something like "in ordinary circumstances, what is the probability that these pixels would be caused by a scene with a dog in them."

**The graphical model underplays the importance of model architecture.** The graphical model shown above represents both questions and answers as unidimensional, and it makes it seem that a small sample is sufficient to get a reasonably good approximation of the true function. In reality the questions are very high dimensional and most architectures fail to get a good fit. Neural nets, especially those with a transformer structure, have had remarkable success in fitting the data, leading to leaps in performance.
