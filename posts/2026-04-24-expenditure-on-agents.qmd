...
New: observations on the returns to tokens.

Theory:

1. Aggregate elasticity is the average of individual elasticities.
2. The Apple-picking model implies low elasticity (for now).
3. The Zeira model implies low elasticity to tokens (for now).
4. Distillation implies low elasticity to tokens
5. Test-time compute implies high elasticity to tokens.
6. Intrinsic complexity can get high elasticity (NP problems)
7. Mythical-man-month, coordination costs
8. If many problems are shared, implies low elasticity.
9. A monopolist would try to make returns elastic.
10. Task model (=Zeira)


Data:

1. Hypothetical: if you had 2X tokens, how much more valuable?
2. Users who don't hit their limits.
3. Demand for AI
4. Benchmark scaling curves
5. Benchmark Pareto frontiers
6. Returns to optimization
7. Cost-to-complete data
8. Changes to benchmark scaling curves & pareto frontiers.
...
: 
    ...
    \begin{tikzpicture}[scale=1,yscale=3]
        ...
        % agent: V = Aa * E^alphaa
        \def\Aa{0.5} \def\alphaa{0.25}

        % --- derived tangencies (E* = (A*alpha/slope)^(1/(1-alpha))) ---

        \draw[-] (0,0) -- (\xmax,0) node[midway,below] {tokens / expenditure (\$)}
            --(\xmax,1.4)--
        ...
        \draw[thick, domain=0:\xmax, samples=120]
            plot (\x, {\Ah*pow(\x,\alphah)}) node[right,align=left] {elastic\\returns};
        \draw[thick, domain=0:\xmax, samples=120]
            plot (\x, {\Aa*pow(\x,\alphaa)}) node[right,align=left] {inelastic\\returns};
        \pgfmathsetmacro{\Estar}{pow(\Aa/\Ah, 1/(\alphah-\alphaa))}
        \pgfmathsetmacro{\Vstar}{\Ah*pow(\Estar,\alphah)}
        %\draw[gray, dashed] (\Estar,0) -- (\Estar,1.4);
    \end{tikzpicture}
    ```

    I find it pretty interesting to think about the shape of this curve, and how to expect it will change in the future. 

There are two extreme perspectives.
: 

    | returns are inelastic                                   | returns are elastic                                |
    | ------------------------------------------------------- | -------------------------------------------------- |
    | doubling your token usage increases your value a little | doubling your token usage increases your value a lot |
    | labs capture a small share of the value                 | labs capture a large share of the value            |
    | customers get much more value than they spend           | customers get a little more value than they spend  |

I'm on team inelastic.
: 
    For the sake of argument I'll say that I think there's strong reason to believe the returns to tokens are *inelastic*, and that this could remain the same all the way to AGI.


#           Observations on the Returns to Tokens

[1] The elasticity is the ratio between expenditure and value.
: 
    Let $K$ be spend on AI, and $F(K)$ is the value of that spend, with $F(0)=0$. We define the elasticity as follows:
    $$\varepsilon(K)= \utt{\frac{dF(K)}{dK}\frac{K}{F(K)}}{elasticity of $F(\cdot)$}{at $K$}
        = \frac{\text{marginal value at $K$}}{\text{average value at $K$}}.$$
    
    At the optimal expenditure, $K^*$, the elasticity will be equal to the ratio of expenditure to value:

    $$\varepsilon(K^*)
        = \frac{\text{expenditure}}{\text{value}}
        = \frac{K^*}{F(K^*)}
        = \utt{\frac{dF(K^*)}{dK^*}\frac{K^*}{F(K^*)}}{elasticity of $F(\cdot)$}{at $K^*$}
        = \frac{\text{marginal value at $K^*$}}{\text{average value at $K^*$}}$$
    
    Because $F'(K^*)=1$. We can illustrate this as follows:
...
[2] If the returns to tokens are inelastic, then demand for tokens is inelastic.
: 
    A buyer chooses $K$ to maximize $F(K) - p K$, so the FOC $F'(K^*) = p$ gives the inverse demand curve $p(K) = F'(K)$.

    1. Implicit differentiation of the FOC gives $\frac{dK^*}{dp} = 1/F''(K^*)$.

    2. The price-elasticity of demand is then

        $$\eta \;\equiv\; -\frac{d\ln K^*}{d\ln p} \;=\; -\frac{F'(K^*)}{K^*\,F''(K^*)} \;=\; \frac{1}{\xi(K^*)},$$

        where $\xi(K) \equiv -K F''(K)/F'(K)$ is the *elasticity of marginal value* (how fast the marginal token's value falls).

    3. For constant-elasticity returns $F(K) = A K^\alpha$, $\xi = 1-\alpha$ everywhere, so

        $$\eta = \frac{1}{1-\alpha}.$$

    | $\alpha$ (value elasticity) | $\eta$ (demand elasticity) |
    |:---:|:---:|
    | $0.2$ (steeply diminishing) | $1.25$ (barely elastic) |
    | $0.5$                       | $2$                     |
    | $0.7$                       | $3.3$                   |
    | $0.9$ (nearly linear)       | $10$                    |

    So inelastic returns deliver a demand curve hugging unit-elasticity; the elastic case is much more elastic. If returns *saturate* ($\alpha$ falling in $K$ rather than constant), demand can drop below unit-elastic outright: the identity $\xi = 1 - \alpha - \frac{d\ln\alpha}{d\ln K}$ shows that decreasing $\alpha(K)$ pushes $\eta$ below $1/(1-\alpha)$.

    Visually, inelastic returns give a *steep* demand curve (a small reduction in quantity requires a sharp price jump), while the elastic case is much flatter:
    ...
    \begin{tikzpicture}[scale=1, yscale=2]
        \def\xmax{5}
        \def\ymax{1.4}
        % parameters: A*alpha = 0.5 for both, so curves cross at (K=1, p=0.5)
        \def\alphai{0.2} \def\Ai{2.5}
        \def\alphae{0.7} \def\Ae{0.7142857}

        \draw[-] (0,0) -- (\xmax,0) node[midway,below] {quantity $K$ (tokens)}
            -- (\xmax,\ymax) --
            (0,\ymax) -- (0,0) node[midway,rotate=90,above] {price $p$};

        \draw[thick, domain=0.3:\xmax, samples=200]
            plot (\x, {\Ai*\alphai/pow(\x,1-\alphai)})
            node[right, align=left] {inelastic\\returns\\($\eta=1.25$)};
        \draw[thick, domain=0.3:\xmax, samples=200]
            plot (\x, {\Ae*\alphae/pow(\x,1-\alphae)})
            node[right, align=left] {elastic\\returns\\($\eta=3.3$)};

        % marker at the common point (1, 0.5)
        \filldraw[gray] (1, 0.5) circle (1.5pt);
    \end{tikzpicture}
    ```
...
[3] The aggregate elasticity is the value-weighted average of individual elasticities.
: 
...
[4] The returns to expenditure appear to be shifting vertically.
: 
    Two recent releases (Opus 4.7 and GPT 5.5) show returns-to-tokens curves that appear to be approximately shifting vertically:
