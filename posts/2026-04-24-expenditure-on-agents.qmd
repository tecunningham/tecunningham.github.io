...
: 
    ...
    2. **This implies the value we get AI will be much larger than the money we spend on AI.** If the returns to AI expenditure remain inelastic (and there are reasons to expect this), then people and firms will get much more value from AI than what they spend on it.
...
%        \pgfmathsetmacro{\Eatop}{(\ytop-\Iai)/\mslope}
    ...
    When we compare the returns on agentic labor vs human labor we often see a "tortoise-hare" relationship: the agent wins at low levels of expenditure, but the human wins with higher levels. I discuss some theory and evidence for this below.
...
: 
    ...
    3. If most people are not constrained on tokens, i.e. they wouldn't use AI much more if the usage was free.

    Not that these are all questions about the expenditure on AI holding fixed the quality of AI. These are all consistent with progression in model quality causing a significant increase in value and a significant increase in expenditure.
...
: 
    ...
    Formally, suppose we have an function $F(K)$, where $K$ is expenditure on agents. If $K$ is chosen optimally, so that the marginal value is equal to the marginal cost ($F'(K^*)=1$) then we have:
    
    $$\utt{\frac{K^*}{F(K^*)}}{ratio of expenditure}{to value}
        = \utt{\frac{dF(K^*)}{dK^*}\frac{K^*}{F(K^*)}}{elasticity of $F(\cdot)$}{at $K^*$}$$
    
    the ratio between value and expenditure ($F(K^*)/K^*$) will be exactly equal to the elasticity of that function at $K^*$.
...
: 
    Suppose the returns to AI remained inelastic, then we would expect the following:
...
: 
    Ideally we would vary expenditure on AI across a variety of domains and measure the output:
    ...
    We would also want try to map out the *demand* for AI. This theory implies that demand for AI should be inelastic.
