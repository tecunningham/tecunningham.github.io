...
*[Note: this is probably textbook material from some field, but I could never find the right textbook. Thanks to Kenneth Hung for suggestions on extension to $n>2$.]*
...
**The loss from using a proxy will be proportional to the *excess* covariance.**
- Suppose you're using income to proxy for race in admissions decisions: the effectiveness of the proxy will not be proportional to the correlation between income and race, but instead proportional to the *excess* correlation conditional on test-scores (and that excess correlation is in fact fairly low).^[abc]
...
\begin{document}
   \begin{tikzpicture}[scale=6]
      ...
      \draw[rotate around={45:(.5,.5)},fill=black,opacity=.1] (.5,.5)
         ellipse (.2 and .4);
      \draw[gray,dashed] (.82,.3)--(1.2,.3) node[right,black] {joint density of race \& test-score among students};

      \draw[rotate around={45:(.5,.5)},black] (.5,.5)
         ellipse (.1 and .2);
      \draw[gray,dashed] (.58,.58)--(1.2,1.2) node[right,black] {Pareto frontier (ellipse) using both test-score and race in admission};
...
3. **Improved quality of predictions will increase the efficiency of tradeoffs.** An improvement to the accuracy of $p(A)$ will shift outwards the set of feasible combinations of `A` & `B`, and will improve the average efficiency of all tradeoffs. Even a model with relatively low precision can have high efficiency if, on the margin, the variance of \(p_B\) is low. This would be the case if, for example, our model was able to identify small variations in \(p_A\) among stories with similar levels of \(p_B\), allowing us to increase \(A\) at a small cost in \(B\).
...
We can define the average outcomes among those selected,
   \[X_i = E[p_i| \sum_iw_ip_i>w^*].\]

And define an objective function over those average outcomes:
...
$$X_1=\sigma_1\sqrt{(1 - \rho^2)(1-\frac{X_j^2}{\sigma_2^2})}
    + \rho\frac{\sigma_1}{\sigma_2}X_j.$$
...
### Increased Variance of All Attributes
...
Suppose we increase the variance of $x_1$. This shifts out the Pareto frontier in the direction of $X_1$. In the bivariate-normal case, with no correlation ($\rho=0$), then we have:
   \[ X_1 = \frac{\sigma_1^2}{\sqrt{\sigma_1^2+w^2\sigma_2^2}}\gamma,
   \ \ \ X_2 = \frac{w\sigma_2^2}{\sqrt{\sigma_1^2+w^2\sigma_2^2}}\gamma\]

and we can see that $\frac{dX_1}{d\sigma_2},\frac{dX_2}{d\sigma_1}<0$, as we can see in the diagram:
...
I believe that this negative effect will hold in the bivariate case irrespective of $\rho$, but the full comparative-static is a little unwieldy. Intuitively, when you have more high-values of $x_1$, then the variation in $x_2$ is relatively less important in determining who's chosen, and so $X_2$ will decline.
...
### Proxy Variables [UNFINISHED]

Now suppose we have a third variables, $x_3$, with covariance matrix:
...
- If $\rho_{score,race}=\rho_{income,race}=0$ then frontier is perfectly flat for race.
- If $\rho_{income,race}=1$ then income is a perfect proxy for race, and it's equivalent to directly observing race, so we get the regular Pareto frontier.
- If $\rho_{income,race}=\rho_{score,race}$
...
Suppose we're trading-off `race` and `score`, we have some Pareto curve (more diversity means sacrificing score). Then suppose we can't use `race` anymore but have to proxy it, e.g. with `income`, we should expect the Pareto curve to shift in, the tradeoff gets worse. Effectiveness of proxy increasing in correlation with race, but decreasing in correlation with score. E.g., suppose income and score had 100% correlation, then . //  How much it gets worse depends on (i) correlation with excluded measure; (ii) lack of correlation with other measure.
...
**Functional form.** The distribution of attributes often won't be normal. However to apply the model we really only need a normal distribution for the *marginal* values of $\bm{x}$, i.e. the ones in the neighborhood of the threshold. If we fit a model with $x_i\sim N(\mu_i,\sigma_i)$ that fits near the threshold, we can write:
   \[X_i=\underbrace{\bar{X_i}}_{\text{inframarginal}}
                     +\underbrace{E[x_i|\sum x_jw_j>v^*]}_{\text{marginal}}.\]
